...
from apps.authentication.utils.sms_utils import check_phone_verification_code
...
class PhoneLoginSerializer(serializers.Serializer):
    ...
    send_date = serializers.CharField(max_length=10, write_only=True)
    biz_id = serializers.CharField(max_length=255, write_only=True)
    ...
    def validate(self, data):
        ...
        send_date = data.get('send_date', None)
        ...
        if verification_code is None:
            raise serializers.ValidationError(
                'A verification code is required to log in.')

        if send_date is None:
            raise serializers.ValidationError(
                'A send_date is required to log in.')
        if biz_id is None:
            raise serializers.ValidationError('A biz_id is required to log in.')

        # check 验证码是否相同
        if check_phone_verification_code(phone_number=phone,
                                         biz_id=biz_id,
                                         send_date=send_date,
            ...
            raise serializers.ValidationError('验证码失败')
...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = (
            'email', 'username', 'password', 'token', 'profile', 'organization'
        )
    ...
    def update(self, instance, validated_data):
        ...
        for (key, value) in validated_data.items():
            ...
            if key == 'username':
                if User.objects.all().filter(username=value):
                    raise serializers.ValidationError('username already exists.')
            elif key == 'email':
                if User.objects.all().filter(email=value):
